...
async def _handle_if_blacklisted(update: Update, context: CustomContext, uid: int) -> bool:
    if uid not in context.pydantic_bot_data.ban_list:
        return False

    ban_data = context.pydantic_bot_data.ban_list.pop(uid)
    ...
    await send_temporary_message(
        ...
        context=context,
        text=text,
        recipient_id=context.pydantic_bot_data.group_chat_id,
        delay_delete=300
    )
...
async def _send_rules_and_schedule_expiry(update: Update, context: CustomContext, uid: int):
    ...
    rules_text = context.pydantic_bot_data.user_joined_message_text.format(update.effective_user.full_name)
