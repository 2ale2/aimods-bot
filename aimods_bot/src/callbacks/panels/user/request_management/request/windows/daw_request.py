...
log = logger.getChild("windows_daw_request")
WRCS = RCS.WindowsRequest
DRCS = WRCS.DAWRequest


async def request_daw_name(update: Update, context: CallbackContext) -> int:
    """Richiede il nome della DAW/del Plugin."""
    ...
    return DRCS.DAW_NAME


async def request_daw_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Richiede il link della DAW/del plugin."""
    ...
    return DECA.DAW_LINK


async def request_daw_version(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Richiede la versione della DAW/del plugin."""
    if not update.callback_query:
    ...
    return DRCA.DAW_VERSION


async def recheck_daw_request(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
...
async def edit_daw_request_detail(update: Update, context: CallbackContext) -> int:
...
async def edited_software_detail(update: Update, context: CallbackContext) -> int:
    ...
    try:
        return await recheck_daw_request(update=update, context=context)
    finally:
        RequestDataManager.update_field(context, "editing", None)


async def daw_backer(update: Update, context: CallbackContext) -> int:
...
async def confirm_daw_request(update: Update, context: CallbackContext) -> int:
