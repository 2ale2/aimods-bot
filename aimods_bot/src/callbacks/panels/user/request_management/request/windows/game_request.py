...
log = logger.getChild("windows_game_request")
WRCS = RCS.WindowsRequest
GRCS = WRCS.GameRequest


async def request_game_name(update: Update, context: CallbackContext) -> int:
    """Richiede il nome del gioco."""
    ...
    return GRCS.GAME_NAME


async def request_game_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Richiede il link del gioco."""
    ...
    return GRCS.GAME_LINK


async def request_game_version(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Richiede la versione del gioco."""
    if not update.callback_query:
        await InputHandler.handle_input(update=update, context=context, detail="link")
    ...
    return GRCS.GAME_VERSION


async def request_game_functionalities(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Richiede le funzionalità del gioco"""
    if not update.callback_query:
    ...
    return GRCS.GAME_FUNCTIONALITIES


async def request_game_steamtools(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Richiede se l'utente accetterebbe anche i file steamtools"""
    if not update.callback_query:
    ...
    await RequestDataManager.request_detail(
        ...
        context=context,
        detail="steamtools",
        back_data="back_functionalities"
    )

    return GRCS.GAME_STEAMTOOLS


async def recheck_game_request(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Mostra il riepilogo finale per la conferma"""
    if update.callback_query:
        data = update.callback_query.data
        if data in ("steamtools_yes", "steamtools_no"):
            await InputHandler.handle_input(update=update, context=context, detail="steamtools")
...
async def game_backer(update: Update, context: CallbackContext) -> int:
    ...
    back_actions = {
        ...
        "back_link": lambda: request_game_link(update=update, context=context),
...
async def confirm_game_request(update: Update, context: CallbackContext) -> int:
