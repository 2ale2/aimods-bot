...
log = logger.getChild("windows_software_request")


async def request_name(update: Update, context: CallbackContext) -> int:
    """Richiede il nome del software."""
...
async def request_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Richiede il link del software."""
...
async def request_version(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Richiede la versione del software."""
    if not update.callback_query:
...
async def request_functionalities(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Richiede le funzionalità del software"""
    if not update.callback_query:
...
async def recheck_request(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Mostra il riepilogo finale per la conferma"""
    if not update.callback_query:
        await InputHandler.handle_input(update=update, context=context, detail="funtionalities")
...
async def edited_detail(update: Update, context: CallbackContext) -> int:
...
async def backer(update: Update, context: CallbackContext) -> int:
    ...
    back_actions = {
        ...
        "back_name": lambda: request_name(update=update, context=context),
        "back_link": lambda: request_link(update=update, context=context),
        "back_version": lambda: request_version(update=update, context=context)
    }
...
async def confirm_request(update: Update, context: CallbackContext) -> int:
