...
async def moderation_router(update: Update, context: CustomContext, path: list[str]):
    if len(path) == 0:
...
async def security_and_filters_router(update: Update, context: CustomContext, path: list[str]):
    if len(path):
        match path[0]:
            case "antispam":
