...
from aimods_bot.src.callbacks.panels.admin.moderation.allow_after.render import render_antispam_links_allow_after_panel
...
async def antispam_link_allow_after_route(update: Update, context: CallbackContext, setting: str, path: list[str]):
    if len(path) == 0:
        await render_antispam_links_allow_after_panel(update=update, context=context, setting=setting)
