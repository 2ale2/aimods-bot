...
class WhitelistManager:
    ...
    def get_whitelist(context: CallbackContext, category: CategoryType) -> List[int]:
        ...
        return get_value(context=context, path=f'moderation.antispam.mention.whitelist.{category}') or []
    ...
    def save_whitelist(context: CallbackContext, category: CategoryType, whitelist: List[int]) -> None:
        ...
        set_value(context=context, path=f"moderation.antispam.mention.whitelist.{category}", value=whitelist)
...
async def view_mention_whitelist(update: Update, context: CallbackContext, category: CategoryType, from_category: bool = False):
    ...
    if not from_category:
        callback_data = "moderation/security_filters/antispam/mention/whitelist/view"
    else:
        callback_data = f"moderation/security_filters/antispam/mention/{category}"
...
async def remove_from_category_whitelist(update: Update, context: CallbackContext, category: CategoryType):
    ...
    await update.effective_message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(
            text="🔙 Indietro",
            callback_data="moderation/security_filters/antispam/mention/whitelist/remove"
        )]]),
        parse_mode=ParseMode.HTML
    )

    context.chat_data["editing_mention_whitelist"] = {
        "action": "remove",
...
async def _handle_remove_preparation(update: Update, context: CallbackContext, text: str):
    ...
    keyboard = KeyboardBuilder.build_category_inline_keyboard(
        "moderation/security_filters/antispam/mention/whitelist/remove/",
        "moderation/security_filters/antispam/mention/whitelist"
    )
    ...
    if action == "add":
        return [
            [InlineKeyboardButton(
                ...
                callback_data="moderation/security_filters/antispam/mention/whitelist/add/"
            )],
            [InlineKeyboardButton(
                text="🔙 Indietro",
                callback_data="moderation/security_filters/antispam/mention/whitelist"
            )]
        ]
    else:  # remove
        return [
            [InlineKeyboardButton(
                ...
                callback_data=f"moderation/security_filters/antispam/mention/whitelist/remove/{category}"
            )],
            [InlineKeyboardButton(
                text="🔙 Indietro",
                callback_data="moderation/security_filters/antispam/mention/whitelist"
